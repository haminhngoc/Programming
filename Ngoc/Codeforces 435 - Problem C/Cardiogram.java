...
public class Cardiogram {
	...
	static int n; // 10^3
	static int[] a; // Positive 10^3. Sum <= 10^3
	...
	public static void solve() {
		...
		char[][] matrix = new char[maxY - minY + 1][sumX];
		...
		int currentY = maxY;
		for (int i = 0; i < n; i++) {
			int increasement = i % 2 == 0 ? -1 : 1;
			char character = i % 2 == 0 ? '/' : '\\';
			for (int j = 0; j < a[i]; j++) {
				matrix[currentY][currentX] = character;
				currentX++;
				if (j < a[i] - 1) {
					currentY += increasement;
				}
			}
		}

		for (int i = 1; i <= maxY - minY; i++) {
			String line = new String(matrix[i]);
